...
router.get("/", auth_1.authMiddleware, async (req, res) => {
    try {
        ...
        const result = await connection_1.pool.query(`SELECT id, nombre, direccion, estado, created_at, updated_at
       FROM sucursales
       WHERE empresa_id = $1 AND estado = TRUE
       ORDER BY nombre ASC`, [empresaId]);
...
router.post("/", auth_1.authMiddleware, async (req, res) => {
    try {
        ...
        const result = await connection_1.pool.query(`INSERT INTO sucursales (empresa_id, nombre, direccion)
       VALUES ($1, $2, $3)
       RETURNING id, nombre, direccion, estado, created_at, updated_at`, [empresaId, nombre, direccion ?? null]);
...
router.patch("/:id", auth_1.authMiddleware, async (req, res) => {
    try {
        ...
        const result = await connection_1.pool.query(`UPDATE sucursales
       SET nombre    = COALESCE($1, nombre),
           direccion = COALESCE($2, direccion),
           estado    = COALESCE($3, estado)
       WHERE id = $4 AND empresa_id = $5
       RETURNING id, nombre, direccion, estado, created_at, updated_at`, [nombre, direccion, estado, id, empresaId]);
        if (!result.rows[0]) {
...
//# sourceMappingURL=SucursalesRouter.js.map