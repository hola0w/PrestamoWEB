...
function rangoWhere(desde, hasta, campo = "created_at") {
    const conds = [];
    const vals = [];
    let i = 1;
    if (desde) {
...
// ─────────────────────────────────────────────
router.get("/cobros", auth_1.authMiddleware, async (req, res) => {
    try {
...
// ─────────────────────────────────────────────
router.get("/prestamos", auth_1.authMiddleware, async (req, res) => {
    try {
        const { desde, hasta } = req.query;
        const cuotaConds = [];
        const vals = [];
        let i = 1;
        if (desde) {
            cuotaConds.push(`cq.fecha_vence >= $${i++}`);
            vals.push(desde);
        }
        if (hasta) {
            cuotaConds.push(`cq.fecha_vence <= $${i++}`);
    ...
    `, vals);
        const porEstado = await connection_1.pool.query(`
      SELECT
        p.estado,
        COUNT(*)                             AS cantidad,
...
// ─────────────────────────────────────────────
router.get("/clientes-deuda", auth_1.authMiddleware, async (_req, res) => {
    try {
        const result = await connection_1.pool.query(`
      SELECT
        c.id,
...
// ─────────────────────────────────────────────
router.get("/cuotas-vencidas", auth_1.authMiddleware, async (_req, res) => {
    try {
        const result = await connection_1.pool.query(`
      SELECT
        c.id                                        AS cliente_id,
...
// ─────────────────────────────────────────────
router.get("/ingresos", auth_1.authMiddleware, async (req, res) => {
    try {
        const { desde, hasta } = req.query;
        const { where, vals } = rangoWhere(desde, hasta, "cq.fecha_pago");
        const porMes = await connection_1.pool.query(`
      SELECT
      ...
      ORDER BY mes DESC
    `, vals);
        const totales = await connection_1.pool.query(`
      SELECT
      ...
      FROM prestamos p
    `);
        const cobradoPeriodo = await connection_1.pool.query(`
...
// ─────────────────────────────────────────────
router.get("/historial/:prestamoId", auth_1.authMiddleware, async (req, res) => {
    try {
...
// ─────────────────────────────────────────────
router.get("/prestamos-lista", auth_1.authMiddleware, async (_req, res) => {
    try {
        const result = await connection_1.pool.query(`
      SELECT
        p.id,
