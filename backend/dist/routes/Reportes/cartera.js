...
function condicionesCartera(req, opciones = {}) {
    const q = req.query;
    ...
    if (esUuidValido(q.sucursalId)) {
        conds.push(`p.sucursal_id = $${i++}`);
        vals.push(q.sucursalId);
    }
    if (esUuidValido(q.cobradorId)) {
        conds.push(`p.cobrador_id = $${i++}`);
        vals.push(q.cobradorId);
    }
    if (esUuidValido(q.zonaId)) {
        conds.push(`c.zona_id = $${i++}`);
        vals.push(q.zonaId);
    }
...
// ─────────────────────────────────────────────
router.get("/general", auth_1.authMiddleware, async (req, res) => {
    try {
...
// ─────────────────────────────────────────────
router.get("/activa", auth_1.authMiddleware, async (req, res) => {
    try {
        const { where, vals } = condicionesCartera(req, { conRangoFecha: true });
...
// ─────────────────────────────────────────────
router.get("/vencida", auth_1.authMiddleware, async (req, res) => {
    try {
        const { where, vals } = condicionesCartera(req, { conRangoFecha: true });
        const q = req.query;
        let rangoCond = "";
        if (q.rangoMora) {
            ...
            if (cond)
                rangoCond = `AND (CURRENT_DATE - cq.fecha_vence) ${cond}`;
        }
...
// ─────────────────────────────────────────────
router.get("/castigada", auth_1.authMiddleware, async (req, res) => {
    try {
        const { where, vals } = condicionesCartera(req, { conRangoFecha: true });
        const whereFinal = where
...
// ─────────────────────────────────────────────
router.get("/por-tipo", auth_1.authMiddleware, async (req, res) => {
    try {
        const { where, vals } = condicionesCartera(req, { conRangoFecha: true });
...
// ─────────────────────────────────────────────
router.get("/por-sucursal", auth_1.authMiddleware, async (req, res) => {
    try {
        const { where, vals } = condicionesCartera(req, { conRangoFecha: true });
        const result = await connection_1.pool.query(`
      SELECT
...
// ─────────────────────────────────────────────
router.get("/por-zona", auth_1.authMiddleware, async (req, res) => {
    try {
        const { where, vals } = condicionesCartera(req, { conRangoFecha: true });
        const result = await connection_1.pool.query(`
      SELECT
...
// ════════════════════════════════════════════════════════════
//# sourceMappingURL=cartera.js.map