...
router.get("/", auth_1.authMiddleware, async (req, res) => {
    try {
        ...
        const result = await connection_1.pool.query(`SELECT id, nombre, estado, created_at, updated_at
       FROM zonas
...
router.post("/", auth_1.authMiddleware, async (req, res) => {
    try {
        ...
        const result = await connection_1.pool.query(`INSERT INTO zonas (empresa_id, nombre)
       VALUES ($1, $2)
       RETURNING id, nombre, estado, created_at, updated_at`, [empresaId, nombre]);
...
router.patch("/:id", auth_1.authMiddleware, async (req, res) => {
    try {
        ...
        const result = await connection_1.pool.query(`UPDATE zonas
       SET nombre = COALESCE($1, nombre),
           estado = COALESCE($2, estado)
       WHERE id = $3 AND empresa_id = $4
       RETURNING id, nombre, estado, created_at, updated_at`, [nombre, estado, id, empresaId]);
        if (!result.rows[0]) {
...
//# sourceMappingURL=ZonasRoutes.js.map