...
export class CobrosService {
  ...
  // ─── Registrar cobro ───────────────────────────────────────
  async registrarCobro(prestamoId: string, montoPagado: number, usuarioId: string) {
    // Solo se puede cobrar si el préstamo existe y no está terminado
    const prestamo = await pool.query(
      `SELECT id, cuota_mensual, estado
       FROM prestamos
       WHERE id = $1 AND estado NOT IN ('PAGADO', 'CANCELADO', 'PENDIENTE')`,
      [prestamoId]
    );
    if (!prestamo.rows[0]) return null;

    const res = await pool.query(
      `INSERT INTO cobros_prestamos
         (prestamo_id, monto_pagado, estado_cobro, created_by)
       VALUES ($1, $2, 'PAGADO', $3)
       RETURNING *`,
      [prestamoId, montoPagado, usuarioId]
    );
    return res.rows[0];
  }
  ...
  // ─── Listar por préstamo ───────────────────────────────────
  async listarPorPrestamo(prestamoId: string) {
    const res = await pool.query(`
      SELECT
        cp.*,
        p.capital,
  ...
  // ─── Cambiar estado ────────────────────────────────────────
  async cambiarEstado(id: string, estado: EstadoCobro, usuarioId: string) {
    const res = await pool.query(
      `UPDATE cobros_prestamos
       SET
         estado_cobro = $1,
         updated_by   = $2,
  ...
  async resumenPorPrestamo(prestamoId: string) {
    const res = await pool.query(`
      SELECT
        p.id                                                              AS prestamo_id,
        p.capital,
