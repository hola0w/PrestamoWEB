...
// ─── Helper ───────────────────────────────────────────────────
function cuotasPorMes(tipoPlazo: string): number {
...
export class PrestamoService {
  ...
  async crearPrestamo(
    ...
    const clienteRes = await pool.query(
      `SELECT score FROM clientes WHERE id = $1 AND estado = 'ACTIVO'`,
      [clienteId]
    );
    if (!clienteRes.rows[0] || (clienteRes.rows[0].score ?? 0) < 600) return null;
  ...
  // ─── Cambiar estado ────────────────────────────────────────
  async cambiarEstado(prestamoId: string, estado: EstadoPrestamo, usuarioId: string) {
    const res = await pool.query(
      `UPDATE prestamos
       SET
         estado     = $1,
