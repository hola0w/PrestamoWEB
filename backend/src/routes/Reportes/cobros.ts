...
function condicionesBase(req: Request) {
  ...
  if (q.desde)      { conds.push(`cp.created_at >= $${i++}`); vals.push(q.desde); }
  if (q.hasta)      { conds.push(`cp.created_at <= $${i++}`); vals.push(q.hasta); }
  if (q.clienteId)  { conds.push(`p.cliente_id = $${i++}`); vals.push(q.clienteId); }
  if (q.prestamoId) { conds.push(`cp.prestamo_id = $${i++}`); vals.push(q.prestamoId); }
  if (q.metodoPago) { conds.push(`pg.metodo_pago = $${i++}`); vals.push(q.metodoPago); }
...
// ─────────────────────────────────────────────
router.get("/recibidos", authMiddleware, async (req: Request, res: Response) => {
  try {
...
// ─────────────────────────────────────────────
router.get("/parciales", authMiddleware, async (req: Request, res: Response) => {
  try {
    const { where, vals } = condicionesBase(req);
...
// ─────────────────────────────────────────────
router.get("/completos", authMiddleware, async (req: Request, res: Response) => {
  try {
    const { where, vals } = condicionesBase(req);
    const whereFinal = where ? `${where} AND cp.estado_cobro = 'PAGADO'` : `WHERE cp.estado_cobro = 'PAGADO'`;
...
// ─────────────────────────────────────────────
router.get("/anticipados", authMiddleware, async (req: Request, res: Response) => {
  try {
    const { where, vals } = condicionesBase(req);
...
// ─────────────────────────────────────────────
router.get("/por-metodo", authMiddleware, async (req: Request, res: Response) => {
  try {
    const { where, vals } = condicionesBase(req);
    const whereFinal = where ? `${where} AND cp.estado_cobro = 'PAGADO'` : `WHERE cp.estado_cobro = 'PAGADO'`;
